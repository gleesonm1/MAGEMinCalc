...
function AdiabaticDecompressionMelting(bulk, T_start_C, P_start_kbar, P_end_kbar, dp_kbar, Frac)
    ...
    for k in eachindex(P)
        ...
        if k > 1
            if Frac > 0
                gv = define_bulk_rock(gv, new_bulk, new_bulk_ox, sys_in, "ig");
                out = point_wise_minimization(P[k-1], T, gv, z_b, DB, splx_data, sys_in);
                s = out.entropy 
            end
            T_save = zeros(3)
            s_save = zeros(3)
            ...
            print(T)
            gv = define_bulk_rock(gv, new_bulk, new_bulk_ox, sys_in, "ig");
            out = point_wise_minimization(P[k], T, gv, z_b, DB, splx_data, sys_in); #PT_minimisation(P[k], T, bulk); #point_wise_minimization(P[k], T, gv, z_b, DB, splx_data, sys_in);
        end  
        println(P[k])      
